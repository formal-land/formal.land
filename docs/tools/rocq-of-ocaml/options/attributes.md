...
We prefix all the attributes of `rocq-of-ocaml` by `coq_`. According to the OCaml syntax, depending on the context, you may use a single `@` or a double `@@`.

## coq_axiom_with_reason
When we cannot import the definition of a value, we can use the `[@coq_axiom_with_reason]` attribute to transform it to a Rocq axiom. For example:
```ocaml
...
```
Note that we must give a reason for the use of `[@coq_axiom_with_reason]` in a string parameter. We define the `axiom` value in the `rocq-of-ocaml`'s Rocq library. The definition of `axiom` is:
```coq
Axiom axiom : forall {A : Set}, A.
```

## coq_cast
With the attribute `[@coq_cast]` we can force the type in Rocq of an arbitrary OCaml expression using the following axiom:
```coq
...
## coq_force_gadt
The `[@coq_force_gadt]` attribute forces a type definition to be considered as a GADT during the translation to Rocq. In particular, it forces the translation to erase the type parameters. For example:
```ocaml
...
## coq_implicit
The `[@coq_implicit "A" "..."]` attribute adds an arbitrary annotation on an OCaml identifier or constructor. We typically use this attribute to help Rocq to infer implicit types where there is an ambiguity:
```ocaml
let forget x = ()
...
For recursive notations, you can combine this attribute with `@coq_struct` to tell `rocq-of-ocaml` to generate a recursive notation. For example, we transform:
```ocaml
type 'a tree =
  | Leaf of 'a
  | Node of 'a tree list
...
The `[@coq_phantom]` attribute forces `rocq-of-ocaml` to consider a type as phantom. This can be useful for abstract types in `.mli` files, since their definition is not reachable. For example:
```ocaml
type 'a num
```
translates to:
...
## coq_plain_module
We may prefer to translate a module to a plain Rocq module rather than a record. The `[@coq_plain_module]` attribute requires a module to be translated as a plain Rocq module. For example:
```ocaml
module type T = sig
  type t
...
## coq_struct
For recursive definitions, we can force the name of the parameter on which we do structural recursion using the attribute `[@coq_struct "name"]`. This has the same effect as the `{struct name}` keyword in Rocq. For example, we translate:
```ocaml
