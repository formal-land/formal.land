...
## Ignored functions
Sometimes definitions are too complex to translate to Rocq, but we still want to go on with the rest of the files. A solution is to add the [coq_axiom_with_reason](options/attributes#coq_axiom_with_reason) to ignore a definition and replace it with an axiom of the same type.
