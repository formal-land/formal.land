...
As an example, `rocq-of-ocaml` translates the following OCaml code:
```ocaml
(* OCaml code *)
let rec sum tree =
  match tree with
...
```
to the corresponding Rocq code:
```coq
(* Rocq code generated by `rocq-of-ocaml` *)
...
```
As you can see, there are no much differences between the two apart from the syntax. For example, we try to never generate new variable names during the translation. Having a Rocq code that is similar in shape and in size to the original program helps us to write formal proofs more efficiently. Following an existing codebase is also a good idea to organize all the proofs when certifying a software.
...
:::tip Offer
If you want to make extra secure OCaml programs (to be applied in domains such as aviation, automotive, or medical devices) with formal verification, you can contact us by email at [&#099;&#111;&#110;&#116;&#097;&#099;&#116;&#064;formal&#046;&#108;&#097;&#110;&#100;](mailto:&#099;&#111;&#110;&#116;&#097;&#099;&#116;&#064;formal&#046;&#108;&#097;&#110;&#100;) ✉️ or schedule a call on [koalendar.com/e/meet-with-formal-land](https://koalendar.com/e/meet-with-formal-land) ☎️. Our main expertise is in the Rocq system. We estimate there should be one *Rocq developer* for each three or four *OCaml developers* to make formally verified software.
:::
